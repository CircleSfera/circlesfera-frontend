...
import type { CreatePostDto, CreateStoryDto, Audio as AudioTrack, StoryElement } from '../types';
    ...
    try {
      const uploadedMedia = await Promise.all(
        mediaFiles.map(async (item, idx) => {
          ...
          try {
            const response = await api.post<{ url: string; type: string }>('/uploads', formData, {
      ...
      if (mode === 'STORY') {
          await Promise.all(mediaFiles.map((_, idx) => 
             createStoryMutation.mutateAsync({
                 mediaUrl: uploadedMedia[idx].url,
                 mediaType: uploadedMedia[idx].type,
                 isCloseFriendsOnly,
                 audioId: selectedAudio?.id,
                 isPremium,
                 price: isPremium ? (Number(price) || 0) : undefined,
             })
          ));
      } else {
          const payload: CreatePostDto & { isPremium: boolean; price?: number } = {
            caption,
            location,
            hideLikes,
            turnOffComments,
  ...
  const reset = () => {
    if (step === 'edit') {
      if (mode === 'STORY' && isComposed) {
        // Instead of going back to upload, go back to composer
        // (This will be handled by the Modal by checking showStoryComposer)
        setStep('upload'); 
        setIsComposed(false);
      } else {
        setStep('upload');
