...
import { Bookmark, Clapperboard, Plus, UserSquare2, Heart, MessageCircle, Grid, Link as LinkIcon, MapPin, ExternalLink, Settings, MoreHorizontal, Flag, Ban, Shield } from 'lucide-react';
            ...
            >
                ...
                {(post.media?.[0]?.type === 'video' || post.type === 'FRAME') ? (
                     <video
                        ...
                        onError={(e) => {
                            console.error('Video load error for post:', post.id, post.media?.[0]?.url);
                            // Only hide if it's really broken, but maybe show a placeholder instead
  ...
  return (
    <div className="min-h-screen pt-4 md:pt-8 pb-32">
      <div className="max-w-3xl mx-auto px-4">
        {/* Profile Card */}
        <div className="glass-panel rounded-3xl md:rounded-4xl p-4 md:p-6 mb-6 overflow-hidden relative group/header">
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8">
                    ...
                    <div className="relative group/avatar shrink-0">
                        <div className="w-16 h-16 md:w-24 md:h-24 rounded-full p-1 bg-white/10 border border-white/20 shadow-xl group-hover/avatar:border-white/40 transition-all duration-500 ring-8 ring-black/40">
                            <div className="w-full h-full rounded-full border-2 border-black overflow-hidden bg-zinc-900">
                                {profile.data.avatar ? (
                                    <img 
                                        src={sanitizeUrl(profile.data.avatar)} 
                        ...
                        {isMe && (
                            <Link 
                                to="/accounts/edit" 
                                className="absolute bottom-1 right-1 p-2 bg-zinc-900 border border-white/10 rounded-full text-white hover:bg-zinc-800 transition-colors shadow-xl opacity-0 group-hover/avatar:opacity-100 translate-y-2 group-hover/avatar:translate-y-0 duration-300"
                            >
                                <Plus size={14} />
                            </Link>
                        )}
                    ...
                    <div className="flex-1 w-full">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
                            <div className="flex flex-col items-center md:items-start text-center md:text-left">
                                <div className="flex items-center gap-2 mb-0.5">
                                    ...
                                    <VerificationBadge level={profile.data.verificationLevel as VerificationLevel} size={18} />
                                </div>
                                <span className="text-xs md:text-base text-zinc-500 font-medium tracking-tight">@{profile.data.username}</span>
                            </div>
                            ...
                            <div className="flex items-center justify-center md:justify-start gap-2">
                                {isMe ? (
                                    <>
                                        <Link 
                                            to="/accounts/edit" 
                                            className="px-5 py-2 bg-white text-black hover:bg-zinc-200 rounded-lg font-bold transition-all hover:scale-105 active:scale-95 text-xs md:text-sm"
                                        >
                                            Edit Profile
                                        </Link>
                                    ...
                                    <>
                                        ...
                                        <button type="button" className="px-5 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg border border-white/5 font-bold transition-all hover:scale-105 active:scale-95 text-xs md:text-sm">
                                            Message
                                        </button>
                        ...
                        <div className="hidden md:flex gap-8">
                            <div className="text-left group/stat cursor-pointer">
                                <span className="block text-white font-black text-lg group-hover:text-brand-primary transition-colors">
                            ...
                            >
                                <span className="block text-white font-black text-lg group-hover:text-brand-secondary transition-colors">
                            ...
                            <button type="button" 
                                ...
                                className="text-left group/stat"
                            >
                                <span className="block text-white font-black text-lg group-hover:text-brand-accent transition-colors">
                ...
                {/* Mobile Stats - Positioned below name section */}
                <div className="flex md:hidden items-center justify-around border-y border-white/5 py-3">
                    <div className="text-center group/stat">
                        <span className="block text-white font-black text-base">
                            {profile.data.user?._count?.posts || 0}
                        </span>
                        <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest leading-none">Posts</span>
                    </div>
                    <button type="button" 
                        onClick={() => setShowFollowsModal('followers')}
                    ...
                    >
                        ...
                        <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest leading-none">Followers</span>
                    </button>
                    <button type="button" 
                        onClick={() => setShowFollowsModal('following')}
                        className="text-center group/stat"
                    >
                        ...
                        <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest leading-none">Following</span>
                    </button>
                </div>

                {/* Bio & Details Section - At the bottom */}
                <div className="max-w-xl text-center md:text-left space-y-4">
                    {profile.data.bio && (
                        <p className="text-zinc-400 text-sm leading-relaxed whitespace-pre-wrap">{profile.data.bio}</p>
                    )}
                    
                    {(profile.data.website || profile.data.location) && (
                        <div className="flex flex-wrap justify-center md:justify-start gap-4 text-xs md:text-sm font-medium">
        ...
        {((highlights?.data && highlights.data.length > 0) || isMe) && (
            <div className="flex items-center gap-4 overflow-x-auto pb-6 mb-6 no-scrollbar border-b border-white/5 scroll-smooth">
                {isMe && (
                    <HighlightBubble 
                        id="new" 
                        title="New" 
        ...
        {/* Tabs */}
        {canView && (
          <div className="border-t border-white/10 flex justify-center gap-8 mb-4">
             <button type="button"
        ...
        {canView ? (
          ...
          <div className="glass-panel rounded-3xl p-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="w-20 h-20 border-2 border-white/20 rounded-full flex items-center justify-center mx-auto mb-6 bg-white/5 shadow-[0_0_20px_rgba(255,255,255,0.05)]">
                <Shield size={32} className="text-white/40" />
             </div>
             <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">This Account is Private</h3>
