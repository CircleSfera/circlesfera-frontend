...
import type { Post, Conversation, Participant } from '../../types';
...
export default function SharePostModal({ isOpen, onClose, post }: SharePostModalProps) {
  ...
  const filteredConversations = conversations.filter(c => {
      ...
      const otherParticipant = c.participants.find((p: Participant) => p.userId !== post.userId);
      ...
      >
        ...
        <div className="relative pt-8 pb-4 px-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={onClose} className="p-2 text-gray-500 hover:text-white transition-colors rounded-full hover:bg-white/5">✕</button>
          </div>
        </div>
                ...
                {isLoading ? (
                    ...
                    filteredConversations.map(c => {
                        const otherParticipant = !c.isGroup 
                            ? c.participants.find((p: Participant) => p.userId !== post.userId)
                            : null;
