...
import { Trash2, MessageCircle, X, Image as ImageIcon, Loader2, Heart } from 'lucide-react';
...
const CommentItem = ({ 
  ...
  const isLiked = comment.likes?.some((l: { userId: string }) => l.userId === currentUserId) ?? false;
  ...
  return (
    <div className={`space-y-3 ${depth > 0 ? 'ml-8 mt-3 relative' : ''}`}>
      ...
      <div className={`flex gap-3 group ${isDeleting ? 'opacity-50' : ''}`}>
        <img
          src={comment.user.profile.avatar || '#noimage'}
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="text-sm">
              <span className="font-semibold text-white mr-1 inline-flex items-center gap-1">
                ...
                <VerificationBadge level={comment.user.verificationLevel as VerificationLevel} size={12} />
              </span>
...
export default function CommentList({ postId, comments }: CommentListProps) {
  ...
  const likeMutation = useMutation({
    mutationFn: ({ commentId, isLiked }: { commentId: string; isLiked: boolean }) => 
      isLiked ? commentsApi.unlike(postId, commentId) : commentsApi.like(postId, commentId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['comments', postId] });
    },
  });
